...
class PongConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_update_dimensions(self, data):
        self.ball.set_ball_dimensions(data.get("ball_width", BALL_DEFAULT_WIDTH), data.get("ball_height", BALL_DEFAULT_HEIGHT))
    ...
    async def handle_start(self, data):
        ...
        if not controllers:
            ...
            await self.send_game_state("✕ Cannot start game: No controllers found in settings!")
            await self.set_game_status(GAME_STATUS_IDLE)
            return
        ...
        if self.game_update_task:
            ...
            for paddle in self.paddles.values():
                paddle.reset()
            self.game_update_task.cancel()
            logger.debug("✓ Game update loop canceled as part of initializing a new loop process")
            await self.set_game_status(GAME_STATUS_ENDED)
