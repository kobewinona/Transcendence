...
import { isVueComponent, svgComponents, tryParseAnyError } from 'shared/lib';
...
const { data: userInfo = {} } = useQuery({
  fetchFn: usersApi.getUserInfo,
  options: {
    select: (res) => res?.data,
    onError: (error) => showErrorModal(error.status, tryParseAnyError(error)),
    onSettled: () => menu.release(),
  },
});

const { mutate: onSignOut, isLoading } = useMutation({
  fetchFn: authApi.signOut,
  options: {
    onSuccess: () => {
      auth.logout();
